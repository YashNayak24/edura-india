...
// ─────────────────────────────────────────────────────────────────
export function useCourses({ category = "all", search = "", initialCourses = [] } = {}) {
  ...
  useEffect(() => {
    if (firstRun.current && initialCourses.length && category === "all" && !search) {
  ...
  }, [category, search, initialCourses]);
...
// ─────────────────────────────────────────────────────────────────
export function useCourse(slug, initialCourse = null) {

  // ✅ Cache se seedha lo — agar cached hai toh instant render
  const [course,  setCourse]  = useState(() => initialCourse || getCachedCourse(slug));
  const [loading, setLoading] = useState(!initialCourse && !getCachedCourse(slug));
  const [error,   setError]   = useState(null);
  ...
  useEffect(() => {
    ...
    if (initialCourse || getCachedCourse(slug)) {
