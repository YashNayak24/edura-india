...
function NavLink({ to, end, className, ...props }) {
  const pathname = usePathname();
  const isActive = end ? pathname === to : pathname.startsWith(to);
  return <Link href={to} className={typeof className === 'function' ? className({ isActive }) : className} {...props} />;
}
    ...
    >
      <div className="max-w-[1260px] mx-auto px-4 sm:px-8 flex items-center h-[68px] justify-between gap-4">
        ...
        <Link href={"/"} className="flex items-center no-underline flex-shrink-0">
          <img src={logo.src} alt="EDURA" className="h-[44px] w-auto object-contain" />
        </Link>
