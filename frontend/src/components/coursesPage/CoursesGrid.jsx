...
const CAT_GRADIENT = {
  "Digital Marketing":     "linear-gradient(135deg,#059669,#34D399)",
  "Web Development":       "linear-gradient(135deg,#DB2777,#F472B6)",
  "Multimedia":            "linear-gradient(135deg,#7C3AED,#A78BFA)",
  "Programming":           "linear-gradient(135deg,#D97706,#FBBF24)",
  "Computer Fundamentals": "linear-gradient(135deg,#0891B2,#67E8F9)",
  "Analyst":               "linear-gradient(135deg,#65A30D,#BEF264)",
  "Designing":             "linear-gradient(135deg,#EC4899,#FCA5A5)",
  "default":               "linear-gradient(135deg,#094E93,#0A5FAD)",
};
    ...
    >
      ...
      <div className={`relative overflow-hidden flex items-center justify-center flex-shrink-0
        ...
        {thumbUrl ? (
          ...
          <div
            className="absolute inset-0 transition-transform duration-[400ms] group-hover:scale-[1.07]"
        ...
        {course.badge && (
          <span
            className="absolute top-3 left-3 z-10 text-[9.5px] font-extrabold tracking-[0.07em]
              uppercase px-[9px] py-[3px] rounded-[5px] text-white backdrop-blur-[8px]"
            style={{ background: badgeBg }}>
            {course.badge}
          </span>
      ...
      <div className={`flex flex-col flex-1 ${listView ? "p-[14px_18px] overflow-hidden" : "p-[16px_18px_18px]"}`}>
        ...
        <div className="flex flex-wrap gap-[6px] mb-3">
          {course.duration && (
            <div className="flex items-center gap-1 bg-[#F4F7FF] border border-[#DDE5F8] rounded-[6px] px-[9px] py-[3px] text-[11px] font-semibold text-[#5A6A8A]">
              <svg viewBox="0 0 24 24" className="w-3 h-3" fill="none" stroke="#5A6A8A" strokeWidth={2}>
...
export default function CoursesGrid({
  ...
  onClearCat, onClearDur, onClearLevel, onClearMode, onClearAll,
}) {
  const [sortBy, setSortBy] = useState("popular");
  const [listView, setListView] = useState(false);

  const catLabel = CATEGORIES.find(c => c.val === cat)?.label;
  const durLabel2 = DURATIONS.find(d => d.val === dur)?.label;

  // ✅ Build active chips list
  const hasAnyFilter =
    cat !== "all" || dur !== "all" || levels.length > 0 || modes.length > 0;
    ...
    <main>
      ...
      <div className="relative mb-4">
        <svg viewBox="0 0 24 24" className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none"
          fill="none" stroke="#8A9ABF" strokeWidth={2}>
          <circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" />
        </svg>
        <input
          type="text"
        ...
        {search && (
          <button
            onClick={() => onSearch("")}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-[#8A9ABF] hover:text-[#0D1E42] bg-transparent border-none cursor-pointer p-0">
            <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.5}>
              <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
            </svg>
          </button>
      ...
      {/* ── Sort bar ── */}
      <div className="flex items-center justify-between bg-white border border-[#DDE5F8]
        rounded-[12px] px-[18px] py-3 mb-4 gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          <span className="text-[13px] font-bold text-[#0D1E42]">Courses</span>
          <span className="text-[12.5px] text-[#5A6A8A]">
            Showing <strong className="text-[#094E93]">{sorted.length}</strong> results
          ...
          <div className="flex gap-1">
            {[
              ...
              <button
                ...
                className="w-8 h-8 rounded-[7px] border-[1.5px] flex items-center justify-center cursor-pointer transition-all duration-150"
                style={{
                  background: listView === mode ? "#094E93" : "#F4F7FF",
                  borderColor: listView === mode ? "#094E93" : "#DDE5F8",
                }}>
                <svg viewBox="0 0 24 24" className="w-[14px] h-[14px]" fill="none"
                  stroke={listView === mode ? "#fff" : "#5A6A8A"} strokeWidth={2}>
                  {icon}
                </svg>
              </button>
            ))}
      ...
      {/* ── ✅ Active filter chips (all filters shown here) ── */}
      {hasAnyFilter && (
        <div className="flex items-center gap-2 flex-wrap mb-[18px]">
          <span className="text-[12px] font-bold text-[#5A6A8A]">Active:</span>

          {cat !== "all" && (
            <FilterChip label={catLabel} onRemove={onClearCat} />
          )}
          {dur !== "all" && (
            <FilterChip label={`Duration: ${durLabel2}`} onRemove={onClearDur} />
          )}
          {levels.map(l => (
            <FilterChip key={l} label={`Level: ${l}`} onRemove={() => onClearLevel(l)} />
          ))}
          {modes.map(m => (
            <FilterChip key={m} label={m} onRemove={() => onClearMode(m)} />
          ))}

          {/* Clear all button */}
          <button
            onClick={onClearAll}
            className="text-[11.5px] font-bold text-[#DC2626] bg-transparent border-none cursor-pointer hover:underline ml-1"
            style={{ fontFamily: "inherit" }}>
            Clear all
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <div className="text-[18px] font-extrabold text-[#0D1E42] mb-2">No courses found</div>
          <div className="text-[14px] text-[#5A6A8A] mb-4">Try adjusting your filters or search term</div>
          {hasAnyFilter && (
            <button
              onClick={onClearAll}
              className="px-5 py-2 bg-[#094E93] text-white rounded-[10px] text-[13px] font-bold border-none cursor-pointer"
              style={{ fontFamily: "inherit" }}>
              Clear all filters
            </button>
          )}
